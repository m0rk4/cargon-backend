...
describe('DriverApplicationService', () => {
  ...
  beforeEach(async () => {
    ...
    id1 = (
      await prismaService.driverApplication.create({
  ...
  describe('getPendingDriverApplications', () => {
    it('should return all driver applications with status "PENDING"', async () => {
      ...
      expect(
        (await driverApplicationService.getPendingDriverApplications()).map(
          (driver) => {
  ...
  describe('createDriverApplication', () => {
    it('should create driver application and return its info', async () => {
      ...
      const driver = await driverApplicationService.createDriverApplication(
        driverApplication,
      );
      driver.createdAt = date;
      driver.updatedAt = date;
  ...
  describe('approveApplication', () => {
    it('should change "PENDING" status of application to "APPROVED"', async () => {
      ...
      expect(
        await driverApplicationService.approveApplication(id1),
      ).toStrictEqual(updateResult);
      expect(
        await driverApplicationService.getPendingDriverApplications(),
      ).toStrictEqual(getResult);
    });
  });
  ...
  describe('declineApplication', () => {
    it('should change "PENDING" status of application to "DECLINED"', async () => {
      ...
      expect(
        await driverApplicationService.declineApplication(id1),
      ).toStrictEqual(updateResult);
      expect(
        await driverApplicationService.getPendingDriverApplications(),
      ).toStrictEqual(getResult);
